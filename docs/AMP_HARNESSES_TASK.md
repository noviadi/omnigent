...
| Task | Status | Notes |
| --- | --- | --- |
| AMP-NATIVE-001 | In progress | Durable versioned delivery journal (`amp_native_delivery.py`) with monotonic CAS transitions (per-record `fcntl` lock, reject regressions) and fail-closed (typed `DurabilityError`) fsync; `submission_started` is verified + fsynced before paste; post-mutation failures become `recovery_required` (never replayable `failed`); executor reconciles non-terminal records to `recovery_required` on restart (no blind replay); confirmation is wired through the real plugin-mirror event path (`confirm_outstanding`, validated by matching thread + `response_id`, idempotent across the recreate→recovery window); durable cross-process set-once dedupe (`external_item_dedupe.py`) keyed by the `agent.start` `response_id`, serialized by a per-key flock critical section that never deletes an active claim, committed after append and recovered against the store on a crashed-mid-append; runner recreate reconciles and publishes a typed recovery event. Second-review hardening (late-confirm-after-recovery, cross-process claim atomicity, marker fsync + unbounded store lookup, verified pre-paste transition, typed durability failures, session-surfaced live-queue refusal) under review. Covered by `tests/test_amp_native_delivery.py`. |
